...
struct GamePadData
{
  ...
  GamePadData(int id,
    ...
    idx(id),
    no_of_axis(axis_count), no_of_buttons(button_count), axis(axis_data), buttons(btns), name(n)
  {
  }
  ...
  bool is_connected =
    false; /**< Is controller connected? Whether or not the controller is connected. */
};
}