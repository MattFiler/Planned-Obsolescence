...
enum class SpriteSortMode
{
  ...
  TEXTURE,       /**< Prepares the render data but does not populate the shared buffers
                      ...
                      and then sorts them by their texture id. This reduces texture
                      and GPU state changes, but breaks the original draw order and is slightly
  ...
  BACK_TO_FRONT, /**< Works identically to texture, but also uses the z-order to control the
                      order of draw calls. All draw calls will be first sorted by their
                      z-order and then sorted by their texture id. This mode will render the sprites
                      with the lowest z-order first. */
  FRONT_TO_BACK, /**< Works identically to texture, but also uses the z-order to control the
                      order of draw calls. All draw calls will be first sorted by their
                      z-order and then sorted by their texture id. This mode will render the sprites
                      with the highest z-order first. */
};
...
class Texture2D
{
  ...
  protected:
  Format format = RGB;            /**< Texture2D Format.
                                       The pixel format used when loading the texture file. */

  unsigned int dims[2]  { 0, 0 }; /**< Texture2D Dimensions.
                                       The dimensions of the loaded texture file. */
};
}