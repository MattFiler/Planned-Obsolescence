...
class Renderer
{
   ...
   */
  Renderer(RenderLib render_lib) : lib(render_lib) {}
   ...
   *  @return The loaded font index.
   */
  virtual int loadFontFromMem(const char* name,
  ...
  virtual void renderText(
    const std::string str, int x, int y, float scale, const Colour& colour, float z_order) = 0;
  ...
  Colour cls =
      COLOURS::STEELBLUE; /**< The clear colour. Used to blank the window every redraw. */

  Colour default_text_colour =
    COLOURS::YELLOWGREEN; /**< The default text colour. Used when no colour is specified. */
};
}